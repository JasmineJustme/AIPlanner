...
async def list_responsibilities(db: AsyncSession = Depends(get_db)):
    ...
    return {
        "code": 200,
        "message": "success",
        "data": _build_tree(items),
    }


@router.post("")
async def create_responsibility(payload: ResponsibilityCreate, db: AsyncSession = Depends(get_db)):
    ...
    return {
        "code": 200,
        "message": "success",
        "data": ResponsibilityResponse.model_validate(item),
    }


@router.put("/{item_id}")
async def update_responsibility(item_id: str, payload: ResponsibilityUpdate, db: AsyncSession = Depends(get_db)):
    item = await db.get(Responsibility, item_id)
    if not item:
...
async def delete_responsibility(item_id: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Responsibility))
    items = result.scalars().all()
    by_parent: dict[str | None, list[Responsibility]] = {}
    target = None
