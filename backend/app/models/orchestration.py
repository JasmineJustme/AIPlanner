...
class Orchestration(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(30), nullable=False, default="analyzing", index=True
    )
    ...
    llm_recommended_name: Mapped[str | None] = mapped_column(
        String(200), nullable=True
    )
    llm_recommended_type: Mapped[str | None] = mapped_column(
        String(20), nullable=True
    )
    llm_recommended_input_params: Mapped[dict | None] = mapped_column(
        JSON, nullable=True
    )
